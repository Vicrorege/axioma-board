...
export const authApi = {
  requestRegister: async (email: string, username: string, password: string) => {
    const res = await axios.post(`${API_BASE}/auth/register-request`, { email, username, password });
    return res.data;
  },
  ...
  resendCode: async (email: string) => {
    const res = await axios.post(`${API_BASE}/auth/resend-code`, { email });
    return res.data;
  },
...
export const mathApi = {
  ...
  explainStep: async (latex: string, title = '', comment = ''): Promise<{
    title: string;
    rule_name: string;
    summary: string;
    ...
    const res = await axios.post(`${API_BASE}/math/explain-step`, { latex, title, comment });
    return res.data;
  },
};
