...
class SympyEngine:
    ...
    def parse(latex_str: str) -> Tuple[Any, bool]:
        ...
        if '=' in cleaned and not any(k in cleaned for k in ['<=', '>=', r'\le', r'\ge', r'\leq', r'\geq']):
            ...
            try:
                ...
                try:
                    lhs = sp.sympify(str(lhs))
                    rhs = sp.sympify(str(rhs))
                except Exception:
                    pass
                ...
                if b == 0:
                    ...
                    if val > 0:
                        ...
                        steps_diff_sq = [
                            ...
                            f"Приравниваем каждый сомножитель к нулю: $({variable} - {latex(sqrt_val)} = 0) \\quad \\text{{или}} \\quad ({variable} + {latex(sqrt_val)} = 0)$",
    ...
    def _get_rational_inequality_breakdown(cls, expr: Any, var_sym: Symbol, variable: str = "x") -> Optional[SolutionMethod]:
        try:
            ...
            frac_f = num_f / den_f
            rel_latex = "\\ge" if str(rel_op) in [">=", "\\ge", "\\geq"] else "\\le" if str(rel_op) in ["<=", "\\le", "\\leq"] else str(rel_op)
            steps.append(f"Запишем неравенство в факторизованном виде: ${latex(frac_f)} {rel_latex} 0$")
            ...
            # 6. Signs on intervals (Dynamically computed from actual real critical points)
            all_crit_points = set()
            if isinstance(num_zeros, sp.FiniteSet):
                for z in num_zeros:
                    try:
                        all_crit_points.add((float(z), z))
                    except Exception:
                        pass
            if isinstance(den_zeros, sp.FiniteSet):
                for z in den_zeros:
            ...
            sorted_crit = sorted(list(all_crit_points), key=lambda item: item[0])
            frac_eval = reduced_frac

            intervals_list = []
            if sorted_crit:
                # 1. Leftmost interval (-inf, p0)
                p0_val, p0_sym = sorted_crit[0]
                t_val = p0_val - 1.0
                try:
                    v_at_t = frac_eval.subs(var_sym, t_val)
                    sign_str = "[+]" if v_at_t > 0 else "[-]"
                except Exception:
                    sign_str = "[+]"
                intervals_list.append(f"(-\\infty, {latex(p0_sym)}) \\; {sign_str}")

                # 2. Intermediate intervals between roots
                for i in range(len(sorted_crit) - 1):
                    pa_val, pa_sym = sorted_crit[i]
                    pb_val, pb_sym = sorted_crit[i + 1]
                    t_val = (pa_val + pb_val) / 2.0
                    try:
                        v_at_t = frac_eval.subs(var_sym, t_val)
                        sign_str = "[+]" if v_at_t > 0 else "[-]"
                    except Exception:
                        sign_str = "[-]" if (i % 2 == 0) else "[+]"
                    intervals_list.append(f"({latex(pa_sym)}, {latex(pb_sym)}) \\; {sign_str}")

                # 3. Rightmost interval (pn, +inf)
                pn_val, pn_sym = sorted_crit[-1]
                t_val = pn_val + 1.0
                try:
                    v_at_t = frac_eval.subs(var_sym, t_val)
                    sign_str = "[+]" if v_at_t > 0 else "[-]"
                    ...
                    sign_str = "[+]"
                intervals_list.append(f"({latex(pn_sym)}, +\\infty) \\; {sign_str}")

                step6_intervals = ", \\quad ".join(intervals_list)
                steps.append(f"Расставим знаки на числовой прямой методом интервалов: ${step6_intervals}$")
            else:
                steps.append("Расставим знаки на числовой прямой методом интервалов")

            # 7. Final solution set
            sol_set = sp.solveset(expr, var_sym, domain=sp.S.Reals)
            ans = f"{variable} \\in {latex(sol_set)}"
            sign_word = "плюс" if any(op in str(rel_op) for op in [">", "\\ge", "\\geq", ">="]) else "минус"
            steps.append(f"Выбираем интервалы со знаком {sign_word}: ${ans}$")
